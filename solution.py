...
#Questions: Do you have to split the data in 2 (in_res, out_res) and train 2 different GPs?
#           Where to use function calculate_cost? (in main() after predictions?)
...
class Model(object):
    ...
    def predict_pollution_concentration(self, test_coordinates: np.ndarray, test_area_flags: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        af = np.asarray(test_area_flags, dtype=bool)

        if np.any(af):
            predictions[af] = gp_mean[af] + gp_std[af]
...
def get_city_area_data(train_x: np.ndarray, test_x: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    train_area_flags[:] = (train_x[:, 2] != 0)
    test_area_flags[:]  = (test_x[:, 2] != 0)
